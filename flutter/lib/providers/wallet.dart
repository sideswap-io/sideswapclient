...
class WalletChangeNotifier with ChangeNotifier {
  ...
  Future<void> _recvMsg(From from) async {
    ...
    switch (from.whichMsg()) {
      case From_Msg.envSettings:
        ref.read(liquidAssetIdProvider.notifier).state =
            from.envSettings.policyAssetId;
        ref.read(tetherAssetIdProvider.notifier).state =
            from.envSettings.usdtAssetId;
        ref.read(eurxAssetIdProvider.notifier).state =
            from.envSettings.eurxAssetId;
        AccountAsset.liquidAssetId = ref.read(liquidAssetIdProvider);
        ref.read(defaultAccountsNotifierProvider).add(
      ...
      case From_Msg.ampAssets:
        ref.read(ampAssetsStateProvider.notifier).state = from.ampAssets.assets;
        break;
      ...
      case From_Msg.priceUpdate:
        final oldPrice = ref.read(walletAssetPrices)[from.priceUpdate.asset];
        final newPrice = from.priceUpdate;
        if (oldPrice == null ||
            oldPrice.ask != newPrice.ask ||
            oldPrice.bid != newPrice.bid) {
          ref.read(walletAssetPrices.notifier).updatePrices(from.priceUpdate);
        }
        break;
      ...
      case From_Msg.serverConnected:
        ref.read(serverConnectionProvider.notifier).state = true;
        break;
      case From_Msg.serverDisconnected:
        ref.read(serverConnectionProvider.notifier).state = false;
      ...
      case From_Msg.gaidStatus:
        ...
        if (gaidStatus.error.isEmpty) {
          if (stokrDetected) {
            ref
                .read(stokrGaidStateProvider.notifier)
                .setState(const StokrGaidStateRegistered());
            return;
          }
          if (pegxDetected) {
            ref
                .read(pegxGaidStateProvider.notifier)
  ...
  void setToggleAssetFilter(String filter) {
    ...
    final allAccounts = ref.read(allAccountsProvider);
  ...
  Future<void> settingsDeletePromptConfirm() async {
    ...
    if (loggedIn) {
      ...
      unregisterPhone();
      await _logout();
      ref.read(pageStatusStateProvider.notifier).setStatus(Status.loading);
      notifyListeners();
    } else {
  ...
  double _getPriceBitcoin(String? assetId) {
    if (assetId == ref.read(liquidAssetIdProvider)) {
      return 1;
    }
    final price = ref.read(walletAssetPrices)[assetId];
    if (price == null) {
      return 0;
    }
  ...
  double _getPriceUsd(String assetId) {
    return _getPrice(ref.read(tetherAssetIdProvider), assetId);
  }
...
final allAssetsProvider =
  ...
  void addTxItem(Map<AccountAsset, List<TxItem>> list,
      AccountAsset accountAsset, TransItem item) {
  ...
  for (final item in allTxs.values) {
    ...
    for (final balance in tx.balances) {
      ...
      addTxItem(list, accountAsset, item);
    }
  }
...
final allAccountsProvider = AutoDisposeProvider<List<AccountAsset>>((ref) {
  ...
  final allAccounts = <AccountAsset>[];
  allAccounts.add(AccountAsset(AccountType.reg, liquidAssetId));
  for (final asset in assets.values) {
    if (asset.swapMarket) {
      allAccounts.add(AccountAsset(AccountType.reg, asset.assetId));
    } else if (asset.ampMarket) {
      allAccounts.add(AccountAsset(AccountType.amp, asset.assetId));
    }
  }
  final remainingAccounts =
      allAssets.keys.toSet().difference(allAccounts.toSet());
...
final allVisibleAccountsProvider =
...
final regularAccountsProvider = AutoDisposeProvider<List<AccountAsset>>((ref) {
  final allVisibleAccounts = ref.watch(allVisibleAccountsProvider);
...
final ampAccountsProvider = AutoDisposeProvider<List<AccountAsset>>((ref) {
  final allVisibleAccounts = ref.watch(allVisibleAccountsProvider);
...
final walletAssetPrices = AutoDisposeNotifierProvider<WalletAssetPrices,
    Map<String, From_PriceUpdate>>(WalletAssetPrices.new);

class WalletAssetPrices
...
final serverConnectionProvider = AutoDisposeStateProvider<bool>((ref) {
  ref.keepAlive();
  return false;
});

...
pub fn try_coin_select_impl(
    ...
    'outer: for with_fee_change in [true, false] {
        ...
        let bitcoin_inputs = if use_all_utxos || bitcoin_user_output == bitcoin_utxos_total {
            ...
            loop {
                let network_fee = network_fee::expected_network_fee_single_wallet(
            ...
            let fee_rate_per_input = if multisig_wallet {
                network_fee::weight_to_network_fee(network_fee::WEIGHT_VIN_MULTI_SIG)
            } else {
                network_fee::weight_to_network_fee(network_fee::WEIGHT_VIN_SINGLE_SIG)
            };
...
pub fn validate_res(
    ...
    let min_network_fee = network_fee::expected_network_fee_single_wallet(
        asset_inputs.len() + bitcoin_inputs.len(),
        args.multisig_wallet,
        user_outputs.len() + change_outputs.len() + usize::from(fee_change.is_some()),
    );
    let max_network_fee = network_fee::expected_network_fee_single_wallet(

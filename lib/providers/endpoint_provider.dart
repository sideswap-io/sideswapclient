      ...
      switch (request.type) {
        EndpointRequestType.newAddress => () {
            ...
            Future.delayed(const Duration(seconds: 1), () {
              final recvAddressAccount = ref.read(walletRecvAddressAccount);
              final recvAddresses = ref.read(walletProvider
                  .select((p) => p.recvAddresses[recvAddressAccount]));

              if (recvAddresses != null) {
                final reply = EndpointReplyModel(
                  reply: EndpointReply(
                    id: id,
                    type: EndpointReplyType.newAddress,
                    data: EndpointReplyDataNewAddress(address: recvAddresses),
                  ),
                );
              ...
              EndpointRequestDataCreateTransaction(
                ...
                () {
                  ...
                  ref.read(paymentProvider).selectPaymentSend(
                      ...
                      address: address);
                  final context =
                      ref.read(walletProvider).navigatorKey.currentContext!;

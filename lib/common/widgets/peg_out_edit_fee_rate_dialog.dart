...
part 'peg_out_edit_fee_rate_dialog.g.dart';

@Riverpod(keepAlive: true)
class PegOutEditFeeRateDialogTransItem
    extends _$PegOutEditFeeRateDialogTransItem {
  @override
  Option<TransItem> build() {
    return none();
  }

  void setState(TransItem transItem) {
    state = Option.of(transItem);
  }
}

class PegOutEditFeeRateDialog extends ConsumerWidget {
  const PegOutEditFeeRateDialog({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return switch (FlavorConfig.isDesktop) {
      true => const DesktopPegOutEditFeeRateDialog(),
      false => const MobilePegOutEditFeeRateDialog(),
    };
  }
}

class DesktopPegOutEditFeeRateDialog extends ConsumerWidget {
  const DesktopPegOutEditFeeRateDialog({super.key});
    ...
    return DPopupWithClose(
      ...
      child: Column(
        children: [
          ...
          DesktopPegOutEditFeeRateDialogContent(),
        ],
      ),
    );
  }
...
class MobilePegOutEditFeeRateDialog extends ConsumerWidget {
  const MobilePegOutEditFeeRateDialog({super.key});
    ...
    return Dialog(
      ...
      child: SizedBox(
        ...
        child: Padding(
          ...
          child: PegOutEditFeeRateDialogContent(),
        ),
      ),
    );
  }
...
class PegOutEditFeeRateDialogContent extends HookConsumerWidget {
  const PegOutEditFeeRateDialogContent({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final optionTransItem = ref.watch(pegOutEditFeeRateDialogTransItemProvider);

    return optionTransItem.match(() => const SizedBox.shrink(), (transItem) {
      final optionOrderId = ref.watch(
        pegOrderIdForTransItemProvider(transItem),
      );

      final optionSelectedFeeRate = ref
          .watch(transItemHelperProvider(transItem))
          .selectedFeeRate();
      final pegOutEditFeeRateHelper = ref.watch(
        pegOutEditFeeRateHelperProvider(optionSelectedFeeRate),
      );
      ...
      useEffect(() {
        if (currentSliderFee.value > defaultFeeRate) {
          currentSliderFee.value = defaultFeeRate;
        }
      ...
      return Column(
        children: [
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                            'New fee: {}'.tr(args: [currentSliderFeeStr]),
                            style: Theme.of(context).textTheme.titleSmall,
                          ),
                          Text(
                            maxFeeStr,
          ...
          CustomBigButton(
            ...
            onPressed: () {
              optionOrderId.match(() {}, (orderId) {
                final msg = To();
                msg.pegEdit = To_PegEdit(
                  orderId: orderId,
                  feeRate: currentSliderFee.value,
                );
...
class DesktopPegOutEditFeeRateDialogContent extends HookConsumerWidget {
  const DesktopPegOutEditFeeRateDialogContent({super.key});

...
class CustomCheckBox extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final frameColorController = useAnimationController(
    ...
    final frameColorAnimation = useAnimation(
      ColorTween(
        begin: frameUnchecked,
        end: frameChecked,
      ).animate(frameColorCurve),
    );

    final backgroundColorController = useAnimationController(
      duration: const Duration(milliseconds: 200),
    );
    final backgroundColorCurve = useMemoized(
      () => CurvedAnimation(
    ...
    final backgroundColorAnimation = useAnimation(
      ColorTween(
        begin: backgroundUnchecked,
        end: backgroundChecked,
    ...
    final fadeAnimation = useAnimation(
      Tween(begin: 0.0, end: 1.0).animate(fadeCurve),
    );

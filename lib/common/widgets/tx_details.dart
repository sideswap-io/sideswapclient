...
class TxDetailsSwapReceived extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final assetImageHelper = ref.watch(assetImageProvider);
    final assetImage = assetImageHelper.getSmallImage(deliveredData.assetId);
...
class TxDetailsNetworkFee extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final assetImageHelper = ref.watch(assetImageProvider);
    final assetImage = assetImageHelper.getSmallImage(networkFeeData.assetId);
...
class TxDetailsBalancesList extends ConsumerWidget {
  const TxDetailsBalancesList(
      {super.key, required this.transItem, this.removeFeeAsset = false});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final balancesData =
        transItemHelper.getBalances(removeFeeAsset: removeFeeAsset);
          ...
          SliverList.builder(
            itemBuilder: (context, index) {
              ...
              final amountColor = amount.contains('+')
                  ? SideSwapColors.menthol
                  : SideSwapColors.bitterSweet;
              final assetImage = assetImageHelper.getSmallImage(assetId);
          ...
          children: [
            ...
            switch (index) {
              ...
              _ => Divider(
                  height: 1,
                  thickness: 1,
                  color: SideSwapColors.glacier.withOpacity(0.4),
                ),
            },
            const SizedBox(height: 12),
            Row(

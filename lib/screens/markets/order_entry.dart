...
class OrderEntryState extends ConsumerState<OrderEntry> {
  ...
  void initState() {
    ...
    ref.read(marketsProvider).subscribeIndexPrice(
        ref.read(requestOrderProvider).tokenAccountAsset().assetId);
  ...
  void updateDollarConversion() {
    ...
    setState(() {
      if (receiveAssetId.assetId == ref.read(tetherAssetIdStateProvider)) {
        receiveConversion = '';
      } else {
        receiveConversion = ref
            .read(requestOrderProvider)
            .dollarConversionFromString(
                receiveAssetId.assetId, receiveController.text);
      }
    });
  }
